...
- **Sort options (per section):** ⭐ แนะนำ (default) / ฿ ราคา / 🎾 คอร์ด / ก–ฮ (Thai) / A–Z (English) — ★ คะแนน ถูกถอดออก 2026-06-12 (rating ไม่แสดงใน UI แล้ว แต่ field ยังอยู่ใน data + ใช้ sort featured ภายใน)
...
- **Map view:** Leaflet + dark CARTO tiles, custom pin markers (**price-tier color fill via `priceTier()`, จุดเปล่าไม่มี label, lime ring if featured**; popup แสดง hours แทน rating) + bilingual price legend, **sport-filter pill** (mapSport state — "all" | "tennis" | "pickleball", **default = "tennis"**, auto `flyToBounds` when switching), **filter row** (type chips + Indoor/AC/Night/BTS toggle pills, reactive markers, filterType auto-resets to "all" when sport switches) **+ hero `searchQ` filters markers too** (same predicate as list, no auto-fit while typing), **geolocation** via `navigator.geolocation.getCurrentPosition` → pulse-blue user marker + `haversineKm()` distance displayed in popup eyebrow; split useEffect + markersRef reactive pattern
- **Detail modal:** Apple-style — gradient header, photo gallery + arrow keys + attribution row; 2×2 info grid (price/hours/**venue indoor-outdoor**/courts — ช่อง rating เปลี่ยนเป็น ในร่ม/กลางแจ้ง 2026-06-12); facilities pills; 3 pill buttons (phone / website / Open in Maps); `body.style.overflow="hidden"` lock
- **Featured (⭐ แนะนำ):** manual selection via `featured:true` field ใน court data (เดิม auto-compute จาก rating+reviews แต่เปลี่ยนเป็น manual 2026-05-18) ผ่าน `isFeatured(court)`; populates the Featured carousel + lime ring บน map markers. ปัจจุบัน 13 courts: tennis 7,20,21,24,26,28,39,40,66 + pickleball 101,102,106,112 (2026-06-12: ถอด id:1 Cozy + id:6 Peninsula, เพิ่ม id:20 Thana City + id:26 Troops + id:39 Crystal Sports)
...
- [x] **Remove rating/reviews from UI** (2026-06-12) — user ตัดสินใจถอด rating/reviews ออกทั้งหมดเพราะอัพเดตให้ทันตลอดไม่ได้: CourtCard (เหลือแค่ราคา), Modal info grid (ช่อง "คะแนน" → "ประเภท" ในร่ม/กลางแจ้ง), map pins (จุดสีเปล่าไม่มี label — แต่ยังมี price-tier color + lime ring), map popup (rating → hours), sort dropdown (ลบ "★ คะแนน"), map subhead อัพเดต. **Field `rating`/`reviews` ยังอยู่ใน data** (ใช้ sort ภายใน featured + ไว้กลับมาใช้ได้) — แค่ไม่แสดงใน UI
- [x] **Featured reshuffle + Thana City refresh** (2026-06-12) — featured: ถอด id:1 Cozy + id:6 Peninsula, เพิ่ม id:20 Thana City + id:26 Troops + id:39 Crystal Sports (รวม 13); id:20 **rename "Thana City Golf & Country Club" → "The Sport Club at Thana City"** + photo ใหม่ + GPS →13.6412368/100.733374 + googleMaps ใหม่ (4gKGCCVBAZUQ4MDQ9 ชี้ listing "The Sport Club at Thana City"); id:6 Peninsula **type hard → grass**; photo updates: ids 9 (RBSC), 12 (Olympic), 14 (Windfield ×2 — URL แรกพลาด), 22 (Sakolpan), 27 (แบริ่ง 68), 30 (สวนหลวง ร.9), 67 (Charcoal)
- [x] **Missing-data hunt รอบ 3** (2026-06-12) — เติม website 3 สนาม pickleball: id:116 Dink A Lot (dinkalotpickleball.com — เว็บทางการ), id:112 Papaya (FB page), id:133 Pickle Park 22 (reclub.co/clubs/@picklepark22 — จองผ่าน Reclub). **Verified:** id:60 Hatch Tennis = สนามเทนนิสจริง venue ใหม่ (IG "Introducing Hatch 🎾"; dataforthai ระบุ บ.แฮทช์คลับ จดทะเบียน badminton rental เป็นแค่ category จดทะเบียน) — ราคา/hours ยังไม่ publish; id:131 MAD DUCK + id:135 PICKLE CLUB + id:137 The Primary ยังไม่มีข้อมูล published เหมือนเดิม; id:139 S9 ยืนยัน 99฿/คน + no reservation (Pickleheads) — hours ไม่มี
- [x] **OG tags + favicon** (2026-06-12) — เตรียมแชร์ลิงก์: meta description + theme-color + Open Graph (og:title/description/url/image/locale th_TH) + Twitter card (summary_large_image) ใน `<head>`; ไฟล์ใหม่ 4 ไฟล์ที่ root: `og-image.png` (1200×630 ดำ+lime สร้างด้วย System.Drawing — regenerate ได้ถ้าจำนวนสนามเปลี่ยน เพราะ embed "85 สนาม"), `favicon.svg` (ลูกเทนนิส lime, primary), `favicon.png` (32px fallback), `apple-touch-icon.png` (180px พื้นดำ); og:image ต้องเป็น absolute URL → ชี้ GitHub Pages

- [x] **เติมรูปครบทุกสนามแล้ว 85/85** (2026-06-12) — หลัง prune 2026-06-11 ทยอยเติม gps-cs-s จาก user จนครบ; id:127 Happy Sports Club 18 (สนามสุดท้ายที่ไม่มีรูป) ถูกลบเพราะไม่มีคอร์ต pickleball จริง; ✅ pickleball batch 1 (2026-06-12): ids 101 (Benchakitti — googleMaps ใหม่ WDcQPVoJxnhL3wmo6, GPS ตรงพินเดิม), 106 (Sterling), 115 (RSC — googleMaps ใหม่ 8zwyzhVdLVhHTZyB7 ชี้ listing "Pickleball @RSC" + GPS →13.7508344/100.4294565 ~63m), 116 (Dink A Lot), 123 (Rally57), 133 (Pickle Park 22), 137 (The Primary); ✅ pickleball batch 2 (2026-06-12): ids 103 (Club 46), 117 (SP), 122 (Backyard), 130 (Rujiseri), 132 (Nawamin LanPat — + googleMaps ใหม่ rU5PvHonLv3bMyem7, GPS ตรงพินเดิม), 138 (Smash!); ✅ **id:6 Peninsula tennis ได้รูปแล้ว (2026-06-12)** — TripAdvisor CDN (`media-cdn.tripadvisor.com/media/photo-w/...`, hotlink ได้ ทดสอบ referer แล้ว, attribution → TripAdvisor search) เพราะ peninsula.com block hotlink + Google Maps listing มีแต่รูป pickleball; ✅ **tennis เติมครบแล้ว 30 สนาม** (2026-06-11, gps-cs-s จาก user, 3 รอบ): ids 2, 7, 8, 11, 13, 15, 17, 18, 19, 20, 21, 23, 24, 29, 31, 32, 38, 39, 40, 43, 46, 47, 49 (พร้อม googleMaps link ใหม่ + GPS ขยับไปพินคอร์ตเทนนิสในสวน 13.814473/100.556988), 53, 55, 57, 60, 62, 64, 65; ✅ id:109 Peninsula pickleball ได้รูปแล้ว (Google Maps "Pickleball court" listing) + GPS แก้ 13.7263→13.7231294 / 100.5113→100.5108485 (~360m, พินจาก listing จริง) — **หมายเหตุ:** id:106 Sterling (pickleball) เป็น venue เดียวกับ id:31 ถ้าอยากใช้รูปร่วมกันก็ได้
...
| 109 | The Peninsula Bangkok | **2026-05-24** photo added (peninsula.com official — ตายแล้ว ถูก prune 2026-06-11); **2026-06-11** photo ใหม่จาก Google Maps "Pickleball court" listing + GPS แก้ →13.7231294/100.5108485 (~360m) |
